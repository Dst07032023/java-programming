...
public class SeparateParts {
        ...
        public static void main(String[] args) {
            ...
            letters.removeIf(p -> !Character.isLetter(p.charAt(0)));//p is every element, every element
            //each character of the String, but those characters were String type, so in order to get a char
            //type we did charAt(0). Character methods needed char type
            System.out.println(letters);

            ArrayList<String> numbers = new ArrayList<>(Arrays.asList(s.split("")));
            numbers.removeIf( str -> !Character.isDigit(str.charAt(0))); //to convert String to char we used
            //charAt(0) to read the only element there
